...
pub fn build_tools_list() -> Vec<Value> {
    vec![
        ...
        tool_def("detect_dead_code", "PRIMARY: Find unreachable functions/methods with zero callers. Use INSTEAD OF manual analysis for dead code cleanup. Excludes entry points and test fixtures.",
            p(true,false,false,json!({})), &["path"]),
        tool_def("trace_callers", "PRIMARY: Find all direct callers of a symbol. Use INSTEAD OF grep for 'who calls this function'. Returns caller symbol IDs, files, and line numbers. Set include_tests=false to exclude callers that are test functions/methods (useful when auditing production call paths). The symbol_id is a two-step lookup: first find it via `search` or `search_symbols` (results include the exact id, e.g. 'app/auth.py::login'), then pass that id here.",
            p(true,true,false,json!({"include_tests":{"type":"boolean","default":true,"description":"Include test-function/method callers. Set false to see only non-test callers."}})), &["path","symbol_id"]),
        tool_def("trace_callees", "PRIMARY: Find all symbols called by a given symbol. Use INSTEAD OF reading function body to find calls. Returns callee symbol IDs, files, and line numbers. Set include_tests=false to exclude callees that are test functions/methods. The symbol_id is a two-step lookup: first find it via `search` or `search_symbols` (results include the exact id, e.g. 'app/auth.py::login'), then pass that id here.",
            p(true,true,false,json!({"include_tests":{"type":"boolean","default":true,"description":"Include test-function/method callees. Set false to see only non-test callees."}})), &["path","symbol_id"]),
        ...
        tool_def("get_code_snippet", "PRIMARY: Get source code for a symbol by ID. Use INSTEAD OF reading files to view function/class source. Returns exact source with context. symbol_id is discovered in two steps: run `search`/`search_symbols` first, then pass the exact id from the results (format 'file/path::name', e.g. 'app/auth.py::login').",
        ...
        tool_def("get_graph_schema", "Show graph schema: node types, edge types, counts, and property names.",
            p(true,false,false,json!({})), &["path"]),
        tool_def("symbol_context", "PRIMARY: Complete context for a symbol in one call — callers, callees, parent scope, file, kind, docstring. Use BEFORE modifying any function to understand its role. symbol_id is discovered in two steps: run `search`/`search_symbols` first, then pass the exact id from the results (format 'file/path::name', e.g. 'app/auth.py::login').",
            p(true,true,false,json!({})), &["path","symbol_id"]),
        ...
        tool_def("find_all_references", "PRIMARY: Find every location where a symbol is referenced. Use INSTEAD OF grep for rename/refactor safety. Default groups by file; set detail=true for per-line calling context. symbol_id is discovered in two steps: run `search`/`search_symbols` first, then pass the exact id from the results (format 'file/path::name', e.g. 'app/auth.py::login').",
