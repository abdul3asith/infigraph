...
pub fn build_tools_list() -> Vec<Value> {
    vec![
        ...
        tool_def("group_build", "PRIMARY: Full group rebuild in one command. Runs: index → sync contracts → link cross-service calls → build combined graph. Use this instead of calling group_index, group_sync, group_link separately. After build, use group_search to query.",
            json!({"group_name":{"type":"string"},"full":{"type":"boolean","default":false,"description":"Clean and rebuild from scratch"}}), &["group_name"]),
